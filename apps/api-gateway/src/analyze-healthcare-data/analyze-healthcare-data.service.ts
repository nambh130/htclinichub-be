...
export class AnalyzeHealthcareDataService {
  ...
  async inputVital(inputVitalDto: InputVitalDto, currentUser: TokenPayload) {
    try {
      const payload = { inputVitalDto, currentUser };

      const result = await firstValueFrom(
        this.httpService.post(
          '/analyze-healthcare/input-vital-signs-data',
          payload,
        ),
      );
    ...
    try {
      const result = await firstValueFrom(
        this.httpService.get(
          `/analyze-healthcare/get-vital-signs-data/${patientId}`,
        ),
      );
      console.log('input Vital API-GATEWAY successfully:', result);
      return result.data;
  ...
  async vitalSignsDataById(id: String, currentUser: TokenPayload) {
    try {
      const result = await firstValueFrom(
