...
import {
  InputVitalDto,
  UpdateVitalDto,
} from '@app/common/dto/analyze-healthcare-data';
import {
  Body,
  Controller,
  Get,
...
export class AnalyzeHealthcareDataController {
  ...
  async inputVitalSigns(
    ...
    try {
      ...
      const analyzeHealthcareData =
        await this.analyzeHealthcareDataService.inputVital(inputVitalDto, user);
  ...
  async vitalSignsDataByPatientId(
    ...
    try {
      const analyzeHealthcareData =
        await this.analyzeHealthcareDataService.getVitalSignsDataByPatientId(
          patientId,
          user,
        );
      return analyzeHealthcareData;
  ...
  async vitalSignsDataById(
    ...
    try {
      const analyzeHealthcareData =
        await this.analyzeHealthcareDataService.vitalSignsDataById(id, user);
      return analyzeHealthcareData;
    } catch (error) {
      console.error('Error input Vital:', error);
      throw error;
  ...
  async updateVital(
    ...
    try {
      const updatedPatient =
        await this.analyzeHealthcareDataService.updateVital(
          id,
          updateVitalDto,
          user,
        );
