...
export class ManageDoctorScheduleService {
  ...
  async getViewWorkingShiftService(doctorId: string, user: TokenPayload) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(
          `/manage-doctor-schedule/doctor-view-working-shift/${doctorId}`,
        ),
      );
  ...
  async getDetailShift(shiftId: string, user: TokenPayload) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(
          `/manage-doctor-schedule/detail-working-shift/${shiftId}`,
        ),
      );
      return response.data;
    } catch (error) {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post(
          `/manage-doctor-schedule/setup-working-shift/${doctorId}`,
          payload,
        ),
      );
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.put(
          `/manage-doctor-schedule/${doctorId}/change-working-shift/${shiftId}`,
          payload,
        ),
      );
  ...
  async getShiftsInDate(date: string, user: TokenPayload) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(
          `/manage-doctor-schedule/doctor/shifts-by-date/${date}`,
          {
            params: {
  ...
  async getShiftsByDoctorIdAndClinicId(
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(
          `/manage-doctor-schedule/doctor/shifts-by-doctor-id-and-clinic-id/${doctorId}/${clinicId}`,
          {
            params: {
              doctorId: user.userId,
            },
