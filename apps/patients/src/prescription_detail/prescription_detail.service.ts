...
export class PrescriptionService {
  ...
  async getPrescriptionsByMRId(mRId: string) {
    try {
      ...
      const medicalRecord = await this.manageMedicalReportRepository.findOne({
        _id: mRId,
      });
  ...
  async createPrescription(
    ...
    const prescription =
      await this.prescriptionRepository.createPrescription(prescriptionData);
    if (prescription) {
      await this.manageMedicalReportRepository.findOneAndUpdate(
        { _id: mRId },
