...
export class LabOrderController {
  ...
  async getQuantResultByOrderItem(@Param('id') id: string) {
    const response = await this.resultService.findQuantitativeResultByOrder(id);
    return response;
  }

  @Get('item/:id/imaging-result')
  async getImagingResultByOrder(@Param('id') id: string) {
  ...
  async saveQuantitativeResult(
    ...
    if (updateResult.deletedResultFiles) {
      this.kafkaClient.emit(
        ...
        new DeleteMultipleFilesEvent({
          currentUser: user,
          ids: updateResult.deletedResultFiles,
        }),
      );
    }
  }
  ...
  async addUploadedResultFile(@Body() dto: AddUploadedFileDto) {
  ...
  async saveImagingResult(
    ...
    if (deletedFiles.length > 0)
      this.kafkaClient.emit(
        ...
        new DeleteMultipleFilesEvent({
          currentUser: user,
          ids: updateResult.deletedImageIds,
        }),
      );
    return updateResult;
  }
