...
export class ManageDoctorScheduleService {
    ...
    try {
        const doctor = await this.doctorRepository.findOne({ where: { id: parseInt(doctorId) } });

        if (!doctor) {
            throw new NotFoundException(`Doctor with id ${doctorId} not found`);
        }

        const shifts = await this.manageDoctorScheduleRepository.find({
            where: { doctor: { id: doctor.id } },
            relations: ['doctor'], // nếu cần join với doctor
            order: { startTime: 'ASC' },
        });
