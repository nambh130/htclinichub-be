...
export class ManageDoctorScheduleService {
    ...
    async getDetailShiftService(shiftId: string, userId: string) {
        if (!shiftId) {
            throw new NotFoundException('Invalid shiftId');
        }

        try {
            const doctor = await this.doctorRepository.findOne({ id: parseInt(shiftId) });

            if (!doctor) {
                throw new NotFoundException(`Doctor with id ${shiftId} not found`);
            }
